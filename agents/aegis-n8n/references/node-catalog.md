...
Referência rápida usada pelo `aegis-n8n` para interpretar tipos de nó e sugerir bibliotecas Python equivalentes. Não é exaustivo: cobre os nós mais comuns. Quando o tipo do nó não estiver listado, deduza pelo nome (`type` segue o padrão `n8n-nodes-base.<servico>` ou `@n8n/n8n-nodes-langchain.<servico>`) e pelo `parameters`.
