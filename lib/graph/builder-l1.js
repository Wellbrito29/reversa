...
export function buildL1(projectRoot, opts = {}) {
  ...
  for (const absPath of targetFiles) {
    ...
    const fns = extractFunctions(ast, rel);
    const cls = extractClasses(ast, rel);
    symbols.push(...fns);
    for (const c of cls) {
      symbols.push({ id: c.id, type: c.type, file: c.file, name: c.name, line: c.line, exported: c.exported, extends: c.extends });
      symbols.push(...c.methods);
    }
    calls.push(...extractCalls(ast, rel));
    exports_.push(...extractExports(ast, rel));
  }
