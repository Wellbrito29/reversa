...
export default async function uninstall(args) {
  ...
  const outputFolder = state.output_folder ?? '_aegis_sdd';
  ...
  // Strip Reversa-installed Keeper hooks before generic file removal.
  ...
  const reversaDir = join(projectRoot, '.aegis');
  try {
