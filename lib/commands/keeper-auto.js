...
import { buildLiteralHints } from '../auto/literal-extractor.js';
import { resolveSpecPath } from '../auto/spec-resolver.js';
import { cleanDeletedRefs } from '../auto/deleted-ref-cleaner.js';
...
export default async function keeperAuto(args) {
  ...
  // Check ANTHROPIC_API_KEY if not dry-run
  if (!opts.dryRun && !process.env.ANTHROPIC_API_KEY) {
    emit(opts.format, {
      ok: false,
      error: 'ANTHROPIC_API_KEY not set. Required for live mode (use --dry-run to skip LLM).',
    });
    process.exit(1);
  }
  ...
  let policy;
  try {
    policy = readAutoPolicy(root);
  } catch (e) {
    emit(opts.format, {
      ok: false,
      error: `Failed to read auto-policy.yaml: ${e.message}`,
    });
    process.exit(1);
  }
  ...
  for (const entry of trimmed) {
    ...
    const specPath = entry.spec_path ?? resolveSpecPath(entry.file, { root });
    if (!opts.dryRun && entry.change_type === 'deleted' && entry.file) {
      try {
        const cleaned = await cleanDeletedRefs(root, entry.file, {
          dryRun: opts.dryRun,
          model: policy.llm.fallback,
        });
        decision.deleted_ref_cleanup = cleaned;
      } catch (e) {
        decision.deleted_ref_cleanup_error = e.message;
      }
    }

    if (!opts.dryRun && decision.route === ROUTE_AUTO && specPath) {
      try {
        const specContent = readFileSync(join(root, specPath), 'utf8');
        const literalHints = buildLiteralHints(entry.diff ?? '', specContent);
        ...
        writeFileSync(join(root, specPath), updated.content, 'utf8');
