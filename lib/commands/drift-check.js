...
// inquirer, or any agent code. Just parses _aegis_sdd/drift.md and
...
function reportText(driftPath, summary, opts) {
  ...
  lines.push('');
  lines.push('Hint: rode `/reversa-keeper after` para resolver, ou ajuste severity para `low`/`medium`.');
  lines.push('');
...
function findMatrix(projectRoot) {
  const candidates = [
    join(projectRoot, '_aegis_sdd', 'traceability', 'code-spec-matrix.md'),
  ];
...
export default async function driftCheck(args) {
  ...
  if (!existsSync(driftPath)) {
    if (opts.format === 'json') {
      ...
      process.stderr.write(`Hint: rode \`/reversa\` no projeto, depois \`/reversa-keeper after\` para popular drift.md.\n\n`);
    }
    process.exit(2);
  }
