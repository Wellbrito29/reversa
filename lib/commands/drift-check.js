  ...
  if (!existsSync(driftPath)) {
    if (opts.format === 'json') {
      process.stdout.write(JSON.stringify({
        ...
        path: driftPath,
        hint: 'Run `/reversa` to initialize, then `/aegis-keeper after` to populate drift.md',
      }, null, 2) + '\n');
    } else {
      process.stderr.write(`\nreversa drift-check: drift.md not found at ${driftPath}\n`);
      process.stderr.write(`Hint: rode \`/reversa\` no projeto, depois \`/aegis-keeper after\` para popular drift.md.\n\n`);
    }
    process.exit(2);
  }
