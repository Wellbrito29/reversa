// `npx aegis-spec policy-index <subcommand>`
//
// Subcommands:
//   build          Build aegis/runtime/context/policy-index.json from spec frontmatter
...
function usage() {
  console.log(`
  Usage: npx aegis-spec policy-index <subcommand>
...
export default async function policyIndexCmd(rawArgs) {
  ...
  if (sub === 'show') {
    ...
    if (!index) {
      console.error(orange('\n  No policy index. Run: npx aegis-spec policy-index build\n'));
      process.exit(1);
    }
