...
//
// Every auto-mode and bot decision lands in `.aegis/audit/YYYY-MM-DD.jsonl`.
// Append-only line-delimited JSON — survives crashes, mergeable across
// processes, easy to ship to a SIEM.
//
// Redaction policy is read from `_aegis_sdd/audit-policy.json` if present:
//
...
function loadPolicy(projectRoot) {
  ...
  const path = join(projectRoot, '_aegis_sdd', 'audit-policy.json');
  let policy = null;
  if (existsSync(path)) {
