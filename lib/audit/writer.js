...
//
// Every auto-mode and bot decision lands in `.reversa/audit/YYYY-MM-DD.jsonl`.
// Append-only line-delimited JSON — survives crashes, mergeable across
// processes, easy to ship to a SIEM.
//
// Redaction policy is read from `_reversa_sdd/audit-policy.json` if present:
//
...
function loadPolicy(projectRoot) {
  ...
  const path = join(projectRoot, '_reversa_sdd', 'audit-policy.json');
  let policy = null;
  if (existsSync(path)) {
