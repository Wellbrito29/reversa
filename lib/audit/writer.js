...
//
// Redaction policy is read from `aegis/config/audit-policy.json` if present:
//
...
import { AUDIT_DIR, AUDIT_POLICY_JSON } from '../paths.js';
...
export function auditLogPath(projectRoot, date = new Date()) {
  ...
  return join(projectRoot, AUDIT_DIR, `${day}.jsonl`);
}

function loadPolicy(projectRoot) {
  if (_policyCacheRoot === projectRoot) return _policyCache;
  const path = join(projectRoot, AUDIT_POLICY_JSON);
  let policy = null;
  if (existsSync(path)) {
