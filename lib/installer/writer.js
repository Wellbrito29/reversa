...
export class Writer {
  ...
  // Cria a estrutura interna .aegis/
  createAegisSpecDir(answers, version) {
    const aegisDir = join(this.projectRoot, '.aegis');
    const configDir = join(aegisDir, '_config');

    this._mkdir(aegisDir);
    this._mkdir(configDir);
    this._mkdir(join(aegisDir, 'context'));
    ...
    const statePath = join(aegisDir, 'state.json');
    ...
    const config = configTemplate
      ...
      .replace('folder = "_aegis_sdd"', `folder = "${answers.output_folder}"`)
      .replace(
        /\[agents\]\r?\ninstalled = \[[\s\S]*?\]/,
        `[agents]\ninstalled = [\n${agentsList}\n]`
      )
    ...
    this._writeNew(join(aegisDir, 'config.toml'), config);
    this._writeNew(join(aegisDir, 'config.user.toml'),
  ...
  _installForwardAssets(aegisDir, answers, version) {
    ...
    // Body templates → .aegis/templates/
    const bodySrc = join(forwardSrc, 'body');
    const bodyDest = join(aegisDir, 'templates');
    ...
    // Scripts sh + ps → .aegis/scripts/{sh,ps}/
    for (const flavor of ['sh', 'ps']) {
      const scriptSrc = join(forwardSrc, 'scripts', flavor);
      const scriptDest = join(aegisDir, 'scripts', flavor);
    ...
    // hooks.yml → .aegis/hooks.yml
    const hooksSrc = join(forwardSrc, 'hooks.yml');
    const hooksDest = join(aegisDir, 'hooks.yml');
    ...
    // setup.json → .aegis/setup.json (com placeholders)
    const setupSrc = join(forwardSrc, 'setup.json');
    const setupDest = join(aegisDir, 'setup.json');
  ...
  // Adiciona _aegis_sdd/ e .aegis/config.user.toml ao .gitignore
  ...
  saveCreatedFiles() {
    const statePath = join(this.projectRoot, '.aegis', 'state.json');
