...
// Adds afterFileEdit hook calling the Aegis Spec runner in lean append-only mode.
//
...
export default {
  ...
  generate({ projectRoot, runnerPath }) {
    ...
    // Phase 1: clean up any old beforeFileEdit Aegis Spec hooks.
    if (Array.isArray(merged.beforeFileEdit)) {
      const filtered = merged.beforeFileEdit.filter((e) => !isAegisSpecHook(e));
    ...
    const filtered = currentAfter.filter((e) => !isAegisSpecHook(e));
  ...
  remove({ projectRoot }) {
    ...
    if (existsSync(hooksPath)) {
      ...
      for (const evt of ['afterFileEdit', 'beforeFileEdit', 'beforeShellExecution', 'afterShellExecution']) {
        ...
        const filtered = data[evt].filter((e) => !isAegisSpecHook(e));
