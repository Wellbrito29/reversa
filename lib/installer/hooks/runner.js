...
//
// Responsibilities:
//   - Append a single JSON line to .reversa/keeper-queue.jsonl per event.
//   - That's it. Heavy work (changelog, drift, spec updates) happens in
//     the Keeper agent batch when /reversa-keeper after runs.
//
// Contract:
//   - Reads JSON payload from stdin (best-effort — empty stdin is OK).
//   - Args: --phase <post|stop> --engine <id> [--tool <name>] [--files <comma>]
...
async function main() {
  ...
  // Accepted phases: post (per-edit), stop (end-of-session).
  // 'pre' is no longer used by Phase 1+ generators (removed for performance).
  if (args.phase !== 'post' && args.phase !== 'stop') return;
    ...
    try {
      ...
      if (existsSync(queuePath)) {
        ...
        if (editLines.length > 0) {
          const files = new Set();
          editLines.forEach((l) => { try { JSON.parse(l).files?.forEach((f) => files.add(f)); } catch {} });
          process.stderr.write(`\n[reversa-keeper] ${editLines.length} edits queued (${files.size} files). Run /reversa-keeper after to update specs.\n\n`);
        }
      }
    } catch { /* swallow */ }
  }
