...
//
// Responsibilities:
//   1. Append the event to .reversa/keeper-queue.json (with file locking)
...
//   - Errors logged to .reversa/keeper-errors.log
//
...
function appendQueueEntry(projectRoot, entry) {
  const queuePath = join(projectRoot, '.reversa', 'keeper-queue.json');
...
function appendChangelogStub(projectRoot, outputFolder, entry) {
  ...
  const stub = [
    '',
    `## ${hh}:${min} — auto-stub (keeper pending)`,
    '',
    ...
    `**Engine:** ${entry.engine}`,
    '',
    `> Stub criado automaticamente pelos hooks. Rode \`/reversa-keeper after\` para enriquecer com motivo, impacto e contexto.`,
    '',
  ].join('\n');
...
function markSpecsPending(projectRoot, outputFolder, specs) {
  ...
  if (existsSync(driftPath)) {
    ...
    content = [
      ...
      'Geração: reversa-keeper hooks v2.0.0',
      '',
      '## Resumo',
      '',
      '_Recompute ao rodar `/reversa-keeper after`._',
      '',
      '## Specs',
      '',
    ...
    for (const spec of specs) {
      ...
      if (line.includes(`\`${spec}\``) || line.includes(`| ${spec} |`)) {
        ...
        if (cells.length >= 6) {
          cells[2] = ` ${ts} `;
          cells[3] = ' 🔴 pending ';
          cells[5] = ' Rodar `/reversa-keeper after` ';
          lines[i] = cells.join('|');
          updated.add(spec);
        }
      }
  ...
  for (const spec of specs) {
    if (updated.has(spec)) continue;
    const row = `| \`${spec}\` | ${ts} | 🔴 pending | — | Rodar \`/reversa-keeper after\` |`;
    lines.push(row);
    updated.add(spec);
  }
...
function warnHighConfidenceImpact(projectRoot, outputFolder, specs) {
  ...
  for (const spec of specs) {
    ...
    if (total > 0 && green / total >= 0.5) {
      if (!warned) {
        process.stderr.write('\n[reversa-keeper] Atenção — specs de alta confiança impactadas:\n');
        warned = true;
      }
  ...
  if (warned) {
    process.stderr.write('  Rode `/reversa-keeper after` antes de commitar para revalidar.\n\n');
  }
}
